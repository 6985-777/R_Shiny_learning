...
server <- function(input, output, session) {
  ...
  # ---------------- Q2 -----------------
  # TR: West Bengal'de (ve yazım varyantlarında) family bazında toplam satış
  # EN: West Bengal families by total sales (dplyr pipeline, integrated from grafik_soru2.R)
  
  output$q2_plot <- renderPlot({
    # DT_* data.table -> dplyr için data.frame'e çeviriyoruz
    df_data   <- as.data.frame(DT_data)
    df_items  <- as.data.frame(DT_items)
    df_stores <- as.data.frame(DT_stores)
    
    # isim farklılıklarına dayanıklı: unit_sales yoksa sales'ı kullan
    names(df_data)
    names(data)
    ...
    # 7. Satışa göre büyükten küçüğe sırala
    ...
    validate(need(nrow(wb_families) > 0, "No data / Veri yok"))
  ...
  output$q2_table <- renderDT({
    ...
    # Gerekli kolonlar: data tarafında item/store anahtarları; items/stores tarafında join anahtarları
    ...
    validate(need(length(missing_data_keys) == 0,
                  paste("df_data eksik kolon(lar):", paste(missing_data_keys, collapse = ", "))))
    
    validate(need("item_nbr" %in% names(df_items),  "df_items içinde 'item_nbr' yok"))
    validate(need("store_nbr" %in% names(df_stores), "df_stores içinde 'store_nbr' yok"))
    
    # df_stores içinde 'state' kolonunu normalize et (varsa farklı isimden state'e çevir)
    ...
    validate(need(length(hit) > 0,
                  "df_stores içinde 'state' (veya state_name/region/province) benzeri bir kolon yok"))
    
    if (hit[1] != "state") {
      df_stores <- dplyr::rename(df_stores, state = dplyr::all_of(hit[1]))
    }
    
    # coalesce için güvenli vektörler
    ...
    wb_families <- df_data %>%
      ...
      # state filtrelemesi (artık df_stores'tan geliyor)
